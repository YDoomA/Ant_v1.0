...
int main()
{
    ...
    Clock clock;                                                                 // Cоздаем переменную времени, т.e. привязка ко времени(а не загруженности/мощности процессора).
    float time = 0;
    ...
    Ant ant0(400,300,2,Color::Blue);                                             // Создаём муравьёв( Это экземпляры класса Ant)
    Ant ant1(400,350,2,Color::Cyan);                                             // отправляем начальные координаты и направление
    Ant ant2(500,150,4,Color::Green);                                            // т.е один Ant муравей2(х=500,у=100, двигает вниз, цвет::серо-бурый);
    ...
    while (app.isOpen())                                                         // Это цикл, который работает пока открыто окно
    {
        ...
        //sf::sleep(sf::milliseconds(2));                                        // Чтоб муравей не бегал слишком быстро, ставим небольшую задержку
        //cout << "\n\n\n\nX : "<< ant0.x <<"\n\nY: "
